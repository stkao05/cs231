...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for j in range(2 * N):
          if j != k:
            d_k += torch.exp(sim(z_k, out[j]) / tau)
        l_k = -torch.log(l_k / d_k)

        l_k_N = torch.exp(sim(z_k_N, z_k) / tau)
        d_k_N = 0
        for j in range(2 * N):
          if j != k+N:
            d_k_N += torch.exp(sim(z_k_N, out[j]) / tau) 
        l_k_N = -torch.log(l_k_N / d_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    deno = norm_left * norm_right # (N, 1)
    dot = torch.sum(out_left * out_right, dim=1, keepdim=True) # (N, 1)
    pos_pairs = dot / deno
...
def compute_sim_matrix(out):
    ...
    norm_out = out / torch.linalg.norm(out, dim=1, keepdims=True) # (2N, D)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pair1 = sim_matrix[torch.arange(0, N), torch.arange(0, N) + N] # (N, )
    pair2 = sim_matrix[torch.arange(0, N) + N, torch.arange(0, N)] # (N, )
    ...
    numerator = (torch.cat([pair1, pair2], dim=0).view(-1, 1) / tau).exp() # (2N, 1)
