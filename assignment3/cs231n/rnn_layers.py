...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    z = x.dot(Wx) + prev_h.dot(Wh) + b  # (N, H)
    next_h = np.tanh(z)
...
def rnn_step_backward(dnext_h, cache):
    ...
    db = np.sum(dz, axis=0)  # (N, H)
    dWh = prev_h.T.dot(dz)  # (H, H)
    dWx = x.T.dot(dz)  # (D, N) * (N, H)
    dprev_h = dz.dot(Wh.T)
    dx = dz.dot(Wx.T)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        x_t = x[:, t, :]
        h_t, cache[t] = rnn_step_forward(x_t, hprev, Wx, Wh, b)
        h[:, t, :] = hprev = h_t
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        ...
        for j in reversed(range(i + 1)):
            cache_j = cache[j]
            dx_j, dh_prev, dWx_j, dWh_j, db_j = rnn_step_backward(
                dh_next, cache_j)
